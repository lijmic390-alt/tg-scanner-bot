...
from config import API_ID, API_HASH, BOT_TOKEN, MY_CHAT_ID, SCAM_PATTERNS, SCAN_HOUR, SCAN_MINUTE if hasattr(__import__('config'), 'SCAN_HOUR') else (9, 0)
...
def is_scam(text: str) -> tuple:
    """Check if message contains scam patterns. Returns (is_scam, matched_pattern)"""
    if not text:
        return False, None
...
async def join_daily():
    """Join new channels and groups every day."""
    async def status(msg):
        await send_msg(f"🔍 {msg}")

    await send_msg(
        f"🔍 Searching for new crypto channels and groups...\n"
...
async def main():
    ...
    # Watch ALL messages from ALL channels and groups
    @user_client.on(events.NewMessage)
    async def watch_messages(event):
        global scam_count
        try:
            # Only from channels and groups
            ...
            chat_type     = "Channel" if isinstance(chat, Channel) and not getattr(chat, "megagroup", False) else "Group"
            ...
            alert = (
                ...
                f"📢 {chat_type} link: t.me/{chat_username}"
            )

            await send_msg(alert)
            log.info(f"Scammer detected: @{sender_username} in @{chat_username}")
    ...
    await send_msg(
        f"✅ SCAMMER DETECTION BOT ONLINE!\n"
        f"{'='*30}\n"
        ...
        f"Currently watching: {joined} channels/groups\n"
        f"Joining 15 more today automatically!\n\n"
        f"Commands:\n"
